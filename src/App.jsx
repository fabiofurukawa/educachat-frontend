...
function App() {
  ...
  // Estado do balão
  const [estiloBalon, setEstiloBalon] = useState("normal");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8faff", position: "relative" }}>
      {/* HEADER */}
      <header className="top-header">
        <img src="/logo.png" alt="EducaChat Logo" className="main-logo" />
      </header>
      
      {/* BOTÕES ABAIXO DO HEADER, NO CANTO DIREITO */}
      <div className="bubble-size-bar">
        <button
          className={`bubble-btn ${estiloBalon === "ultracompacto" ? "active" : ""}`}
          onClick={() => setEstiloBalon("ultracompacto")}
          title="Pequeño"
        >
          P
        </button>
        <button
          className={`bubble-btn ${estiloBalon === "intermediario" ? "active" : ""}`}
          onClick={() => setEstiloBalon("intermediario")}
          title="Medio"
        >
          M
        </button>
        <button
          className={`bubble-btn ${estiloBalon === "normal" ? "active" : ""}`}
          onClick={() => setEstiloBalon("normal")}
          title="Grande"
        >
          G
        </button>
